...
export class SummaryBugChartComponent implements OnInit {
  ...
  ngOnInit() {
    this.picker.updateData$.subscribe(() => {
      this.lineChartLabels = this.picker.currentSprint.bugStats[this.valid ? 'valid' : 'invalid']
        .map((bug: any) => bug.labels)
        .filter((value, index, self) => {
          return self.indexOf(value) === index;
        }).sort();
      this.lineChartData = this.lineChartLabels.map(label => {
        return { data: [this.picker.currentSprint.bugStats[this.valid ? 'valid' : 'invalid'].filter(a => a.labels.includes(label)).length], label: `${label}`}
      });
